...
export class ExamService {
  ...
  getAllQuestionsAndChoices(questionIds: number[]): Observable<QuestionDetails[]> {
  const requests = questionIds.map(id =>
    this.http.get<QuestionDetails>(`${this.baseUrl}/questions/${id}`)
  );
  return forkJoin(requests); // Batch all requests and return as a single observable
  }
